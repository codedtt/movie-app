...
interface MovieCardProps {
  ...
  onWatchTrailer: (active: boolean) => void; // Update: pass the desired state
}

const YOUTUBE_API_KEY = import.meta.env.VITE_YOUTUBE_API_KEY; // Make sure this is set
...
const MovieCard: React.FC<MovieCardProps> = ({
  ...
  // Fetch trailer when user activates trailer
  ...
  // Handle trailer open/close logic
  const handleTrailerToggle = async () => {
    if (!isTrailerActive) {
      await fetchTrailer();
      onWatchTrailer(true); // Open trailer
    } else {
      setTrailerId(null);
      setTrailerError('');
      setLoadingTrailer(false);
      onWatchTrailer(false); // Close trailer
    }
  };

  // Optionally, close trailer from overlay X button
    ...
    <div className="bg-white/10 rounded-2xl p-6 shadow-lg relative text-white flex flex-col md:flex-row gap-6">
      ...
      <div className="flex flex-col flex-1">
        ...
        <div className="flex flex-wrap gap-4 mb-4">
          ...
          {movie.Ratings && movie.Ratings.map((rating: any) => (
